        ...
        foreach ($data->getLigneDeCommandes() as $product){
           dd( $product->getProducts()->getMenus()->getProduits());
            if ($product->getProducts()->getVariety() && $product->getProducts()->getVariety()->getStock()>0) {
                ...
                return new JsonResponse(['error'=>'Stock de '.$product->getProducts()->getVariety()->getStock().' insuffisant, il reste '.$product->getProducts()->getVariety()->getStock()], Response::HTTP_BAD_REQUEST);
                }
            }
