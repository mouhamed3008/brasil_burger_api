    ...
    public function persist($data, array $context = [])
    {
        ...
        foreach ($data->getLigneDeCommandes() as $product){
            if ($product->getProducts()->getVariety() && $product->getProducts()->getVariety()->getStock()>0) {
                dump($product->getQty());
                dd( $product->getProducts()->getVariety()->getStock());
                if ($data->getQty() > $product->getProducts()->getVariety()->getStock()) {
                return new JsonResponse(['error'=>'Stock insuffisant, il reste '.$product->getProducts()->getVariety()->getStock()], Response::HTTP_BAD_REQUEST);
                }
            }
